...
public struct CompositionalView<Content,Item,ID>: View where Content: View,ID: Hashable,Item: RandomAccessCollection,Item.Element: Hashable {
    var content: (Item.Element)->Content
    var items: Item
    var id: KeyPath<Item.Element,ID>
    var spacing: CGFloat
    
    public init(items: Item, id: KeyPath<Item.Element,ID>, spacing: CGFloat = 5, @ViewBuilder content: @escaping (Item.Element)->Content) {
...
struct CompositionalView_Previews: PreviewProvider {
    static var previews: some View {
        ScrollView {
            CompositionalView(items: 1...100, id: \.self) { item in
                ZStack{
                    Rectangle()
                        .fill(.cyan)
